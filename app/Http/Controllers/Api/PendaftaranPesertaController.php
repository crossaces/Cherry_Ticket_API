...
class PendaftaranPesertaController extends Controller
{
    ...
    {
        $PendaftaranPeserta = PendaftaranPeserta::with('event.jenisacara','event.genre','event.kota','event.tiket','order.tiket')->where("ID_PESERTA", "=", $id)->orderBy('ID_PENDAFTARAN', 'DESC')->where('event.TATUS_EVENT', NULL)->get();    
